        ...
        foreach ($vulnerabilities as $vulnerability) {
            ...
            switch ($this->outputType) {
                ...
                default:
                    ...
                    WP_CLI::line(sprintf('Fixed in: %s', array_key_exists('fixed_in', $vulnerability) ? $vulnerability['fixed_in'] : 'fix n/a'));
                    break;
            }
        }
            ...
            foreach ($vulnerabilities as $vulnerability) {
                ...
                switch ($this->outputType) {
                    ...
                    default:
                        ...
                        WP_CLI::line(sprintf('Fixed in: %s', empty($vulnerability['fixed_in']) ? 'fix n/a' : $vulnerability['fixed_in']));
    ...
    private function isVersionLessThan($versionToCheck, $minimumVersion)
    {
        ...
        foreach ($toCheckParts as $index => $version) {
            ...
            if ($index > (count($minimumParts) - 1)) {
                throw new \UnexpectedValueException(sprintf('Cant compare %s to %s', $versionToCheck, $minimumVersion));
            }
